...
func TestBlockService_Offline(t *testing.T) {
	...
	assert.Equal(t, errors.ErrUnavailableOffline.Message, err.Message)
...
func TestBlockService_Online(t *testing.T) {
	...
	t.Run("orphaned block", func(t *testing.T) {
		pbIdentifier := types.ConstructPartialBlockIdentifier(block.BlockIdentifier)
		mockClient.On("Block", ctx, pbIdentifier).Return(nil, polygon.ErrBlockOrphaned).Once()
		b, err := servicer.Block(ctx, &types.BlockRequest{
			BlockIdentifier: pbIdentifier,
		})

		assert.Nil(t, b)
		assert.Equal(t, errors.ErrBlockOrphaned.Code, err.Code)
		assert.Equal(t, errors.ErrBlockOrphaned.Message, err.Message)
		assert.Equal(t, errors.ErrBlockOrphaned.Retriable, err.Retriable)
	})
